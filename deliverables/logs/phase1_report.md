...
実行日: 2026-09-07 15:30 UTC (re-run) / ホスト: pub400.com / ユーザー: ASHIBATA
専用ライブラリ: **ASHIBATA1** (ソース・SAVF・コンパイルリスト), **ASHIBATA2** (オブジェクト・データ)
共有ライブラリへの書込・上書きは行っていない。最初に ASHIBATA1/ASHIBATA2 の状態を記録し、ビルドスクリプトが管理する VCF400 オブジェクトだけを置換して再ビルドした。ASHIBATA2 の既存 PF `ASHIBATA` は保持した。
...
- ビルドスクリプトが管理する ASHIBATA2 の VCF400 オブジェクトを置換。既存の PF `ASHIBATA` は削除していない。
- ASHIBATA1 の各ソース PF へ今回のソースをアップロードし、`BUILDVCF` を生成して 1 ジョブでビルドした。SAVF オブジェクトは保持した。
...
| 手順 | 結果 | ログ |
|---|---|---|
| `savf/VCFV1R3.SAVF` を FTP (binary) で `/QSYS.LIB/ASHIBATA1.LIB/VCFV1R3.FILE` へ転送 | 成功 (5,892,480 bytes, curl exit 0) | `phase1_02_ftp_savf.log` |
| `DSPSAVF` で内容確認 | SAVLIB VCF, 2024-04-04 20:40:52, TGTRLS V4R4M0, 11,160 レコード | `phase1_03_dspsavf.log` |
| `RSTLIB` / `RSTOBJ` | **今回未実施** (既知の CPD0032。前回ログを保持) | `phase1_04_rstlib.log` |
...
PUB400 の一般ユーザーには RSTLIB/RSTOBJ 権限が付与されていないことが前回ログで確認済み (CPD0032)。今回は再試行せず、ユーザー指示どおりソースビルドを実行した。
...
- コンパイルリストは QSYSPRT を捕捉し `pub400_listings/<OBJ>.txt` に分割保存 (45 件)。CRLF の listing を分割できるよう awk に CR 除去を追加した。
...
ASHIBATA2 のオブジェクトインベントリは **62 件** (`phase1_01_prepare.log`,
`phase1_05_build.log`) で、既存 PF `ASHIBATA` を含む。

## 2. VCFMAIN からの主要機能起動 (5250 / tn5250 自動操作)

ドライバ: `tn5250_driver.py` (pexpect + pyte + tn5250 0.19.0)、シナリオ: `phase1_runtime_check.py`、ログ: **`phase1_06_runtime_5250.log`** (全入力 + 画面スナップショット + DB スナップショット)。
...
| b | 9705 / ASHIBATA / 1 + F5 | `Your vote has been RECORDED` → 確認画面 | **追加 (9705,1,ASHIBATA)** |
| c | 9705 / ASHIBATA / 2 + F5 (重複バッジ) | `You have already voted.` | 変化なし |
| d | 9706 / ASHIBATA / 9 + F5 (存在しないアワード) | `Award does not exist.` | 変化なし |
| e | 9706 / NOSUCH / 1 (MM2024) | `Exhibit does not exist` | 変化なし |
| f | 9706 / (空) / 1 (MM2024) | `Must enter Exhibit ID` | 変化なし |
| g | 9706 / NOVOTE / 1 (MM2024; ELIGIBLE=0 のテスト展示) | `Exhibit ineligible for award` | 変化なし |
| h | 9706 / DEMO400 / 2 (MM2024) | `Your vote has been RECORDED` | **追加 (9706,2,DEMO400)** |
...
| 名前 `Devin Tester` / 展示 ASHIBATA(固定) / コメント `Phase1 runtime check badge 9705` + F5 | `Thank you for commenting on this exhibit` (ENDCMT) | **追加 CMTID=3, VISIBLE=Y** |
