...
実行日: 2026-09-06 (UTC) / ホスト: pub400.com / ユーザー: ASHIBATA
専用ライブラリ: **ASHIBATA1** (ソース・SAVF・コンパイルリスト), **ASHIBATA2** (オブジェクト・データ)
共有ライブラリへの書込・上書きは行っていない。ASHIBATA2 の既存データは再ビルド前に `ASHIBATA1/BKPA2` (SAVF) に退避済み。
...
- コンパイルリストは QSYSPRT を捕捉し `pub400_listings/<OBJ>.txt` に分割保存 (46 件)。
...
| RPG 失敗 (2) | **PRINTER** (RNF7030/RNF7260: PRTCMD 未定義・INITP 不正 — 元ソースの欠陥), **BEEMOVIE** (RNF7055: *LOVAL SETLL が非キー付きファイル — 元ソースの欠陥)。いずれも主要機能外 (印刷ユーティリティ・娯楽) |
| CL (5) | 全件作成 (VOTESTUB, ADDGBSTUB, READGBSTUB, VCFSTUB, LRN400STUB) |
| コマンド (4) | 全件作成 (STREXHB, STREXHBEDT, STRVOTERPT, STRCMTEDT) |

**ソース修正 (1 件)**: `QRPGLESRC/PRTLSTVOTE.rpgle` の `FVOTINGDB IF E DISK` → `FVOTINGDB IF E K DISK`。同プログラムは `*LOVAL SETLL VOTINGDB` を使用しており、キー付き宣言がないとコンパイルエラー (RNF7055) になるため (BEEMOVIE と同種の RNF7055)。修正後の再ビルドで severity 00 (`pub400_listings/PRTLSTVOTE.txt`)。

## 2. VCFMAIN からの主要機能起動 (5250 / tn5250 自動操作)

ドライバ: `tn5250_driver.py` (pexpect + tn5250)、シナリオ: `phase1_runtime_check.py`、ログ: **`phase1_06_runtime_5250.log`** (73 画面スナップショット + 全入力 + DB スナップショット)。
...
| b | 4992 / ASHIBATA / 1 + F5 | `Your vote has been RECORDED` → 確認画面 | **追加 (4992,1,ASHIBATA)** |
| c | 4992 / ASHIBATA / 2 + F5 (重複バッジ) | `You have already voted.` | 変化なし |
| d | 4993 / ASHIBATA / 9 + F5 (存在しないアワード) | `Award does not exist.` | 変化なし |
| e | 4993 / NOSUCH / 1 (MM2024) | `Exhibit does not exist` | 変化なし |
| f | 4993 / (空) / 1 (MM2024) | 展示 ID 必須エラー | 変化なし |
| g | 4993 / NOVOTE / 1 (MM2024; ELIGIBLE=0 のテスト展示) | `Exhibit ineligible for award` | 変化なし |
| h | 4993 / DEMO400 / 2 (MM2024) | `Your vote has been RECORDED` | **追加 (4993,2,DEMO400)** |
...
| 名前 `Devin Tester` / 展示 ASHIBATA(固定) / コメント `Phase1 runtime check badge 4992` + F5 | `Thank you for commenting on this exhibit` (ENDCMT) | **追加 CMTID=6, VISIBLE=Y** |
...
```
VOTINGDB  before: (1,1,ASHIBATA) (28,2,ASHIBATA) + 前回実行分 (3974,3975,4322,4323,4665,4666)
VOTINGDB  after : + (4992,1,ASHIBATA) (4993,2,DEMO400)   ← 資格外・重複・不存在アワードは未書込
GUESTBKDB after : + (6,Y,ASHIBATA,'Devin Tester','Phase1 runtime check badge 4992')
SETTINGS        : ADMPSWRD=VCF2024, ALWVOTE=Y (テスト後に復元)
```
