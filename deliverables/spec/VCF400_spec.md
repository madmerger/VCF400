...
5. **UI は全面日本語** (利用者指示 2026-09-07)。ラベル・ボタン・案内文・メニュー項目名・エラー文言は日本語で表示する。ただし台帳 (第 7 章) で保持対象の画面タイトル英文 ("AS/400 DEMO MENU", "WELCOME TO...", "VCF/400 - How to Navigate", "NOMINATE EXHIBIT FOR AWARD", "GUESTBOOK/400 ...", "LEARN/400") は英文をそのまま表示し日本語副題を併記する。エラー行は日本語文言を主表示とし、第 6 章の英語原文 (M-xx) を同じ行に小さく併記する (クロス検証は英語原文 / メッセージ ID で照合する)。DB 内のデータ (展示名・アワード説明・コメント本文・LEARN/400 ページ本文) は翻訳しない。
