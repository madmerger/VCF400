...
public final class Database {
    ...
    private func isEmpty(_ table: String) throws -> Bool {
        try query("SELECT COUNT(*) FROM \(table)") { $0.int(0) }.first == 0
    }
    ...
    public func setSetting(_ key: String, value: String) throws {
        try execute("UPDATE SETTINGS SET VALUE=? WHERE SETTING=?", [value, key])
    }
