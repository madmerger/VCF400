...
public class AdminSecurityOfficerService {
    ...
    public AdminSecurityOfficerService(SecurityOfficerRepository officers) {
    ...
    public String add(String profile) {
        ...
        if (officers.existsById(profile)) {
            return RpgMessages.USER_EXISTS;
        }
        officers.save(new SecurityOfficer(profile));
        return RpgMessages.STATUS_UPDATED;
    }

    /** RPG: ADMOFRLIST copied add logic; BR-ADMADDSOFR-02. */
    public List<SecurityOfficer> list() {
